...
public class SitioDelegate {
	...
	public String updateSitio(Tmio1Sitio nuevo) {
		ResponseEntity<TransactionBody<Tmio1Sitio>> response= null;
		try {
			response= rest.exchange(REST_URI+"/sitio/update", HttpMethod.POST,null, new ParameterizedTypeReference<TransactionBody<Tmio1Sitio>>() {
			});
		}catch (HttpStatusCodeException e) {
			int statusCode=e.getStatusCode().value();
			System.out.println("ERROR: " + statusCode+ " "+ e.getResponseBodyAsString());
	...
	public void delete(Tmio1Sitio sitio) {
		...
		ResponseEntity<TransactionBody<Tmio1Sitio>> response= null;
		try {
			response= rest.exchange(REST_URI+"/sitio/delete", HttpMethod.DELETE,request, new ParameterizedTypeReference<TransactionBody<Tmio1Sitio>>() {
			});
		}catch (HttpStatusCodeException e) {
			int statusCode=e.getStatusCode().value();
			System.out.println("ERROR: " + statusCode+ " "+ e.getResponseBodyAsString());
